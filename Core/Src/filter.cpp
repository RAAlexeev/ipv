...
void filter( int16_t  *pSrc,float32_t *pDst,uint32_t  blockSize){
	float32_t * bufIn_f32 =new float32_t[blockSize], *state = new float32_t[8];
...
//,*bufOut_f32=new(float32_t[blockSize]);
	...
	for(uint16_t i =0; i < blockSize;i++){
		bufIn_f32[i]= pSrc[i]-aver;
	}
	arm_biquad_cascade_df1_f32(&S,bufIn_f32,pDst,blockSize);

	//q31_t * buf_Q31=(q31_t*)((void*)buf);
	//arm_float_to_q31(bufOut_f32,buf_Q31,LEN);
	delete[] state;
	delete[] bufIn_f32;// delete(bufOut_f32);
#ifdef DEBUG
	float32_t sq=0,sqr;
	for(uint16_t i =0; i < blockSize;i++){
		sq+= pDst[i]*pDst[i];
